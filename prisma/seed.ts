...
'''`.trim(),
  ...
  await prisma.prompt.create({
    data: {
      ...
      template:
        `Seu papel é gerar uma recomendação de movimentação de ações, com base nas informações informadas na transcrição abaixo:

O retorno deve seguir o seguinte formato:
'''
Recomendação.

Tabela com as ações, Dividend Yield se disponível e link para algum site com mais informações da ação ...
'''
